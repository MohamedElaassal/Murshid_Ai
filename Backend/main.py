...
async def process_photo_update(chat_id: str, photo_array: list, db: Session):
    ...
    async with httpx.AsyncClient() as client:
        ...
        if not file_data.get("ok"):
            logger.error("Failed to get file from Telegram")
            return
            
        file_path = file_data["result"]["file_path"]
        
        # Download image into byte buffer
    ...
    await send_telegram_message(chat_id, farmer_reply)
        ...
        if "text" in message and message["text"].startswith("/"):
            if message["text"].startswith("/start"):
                background_tasks.add_task(
                    ...
                    chat_id, 
                    "Mrhba bik f Mourchid-AI. Sifet liya tiswra dyal zzra3a dyalek u ghadi n3tik tashkhis u khotta d 3ilaj f Darija. Ma3ndkch ghir tsifet sura."
                )
            return {"status": "ok"}
